...
```
- Notations:
  - infix notation: If a method takes only one parameter, you can use the infix notation, which is more readable. For 
  example:
```scala
// Instead of this
person.likes("movie")
...
person1.greet()  // Prints: Hello from John!
```
...
constructor parameters, you can extends only one class, but you can implement more traits with keyword "with", 
...
also package object that hold standalone methods/constants - there can be just one for package
- Functions:
